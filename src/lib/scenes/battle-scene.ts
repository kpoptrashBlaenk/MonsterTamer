...
import {calculateExpGainedFromMonster, handleMonsterGainingExperience, StatChanges} from "../../utils/leveling-utils";
...
export class BattleScene extends BaseScene {
    ...
    init(data: BattleSceneData) {
        ...
        const chosenBattleSeenOption: BattleSceneOptions | undefined = dataManager.getStore.get(DATA_MANAGER_STORE_KEYS.OPTIONS_BATTLE_SCENE_ANIMATIONS)
    ...
    create() {
        ...
        this.activePlayerMonsterPartyIndex = this.sceneData.playerMonster.findIndex((monster) => monster.currentHp > 0)
    ...
    private enemyAttack(callback: () => void): void {
        ...
        this.battleMenu.updateInfoPaneMessagesNoInputRequired(
            `${this.activeEnemyMonster.name} used ${this.activeEnemyMonster.attacks[this.activeEnemyAttackIndex].name}`,
            () => {
                this.time.delayedCall(1200, () => {
                    this.time.delayedCall(100, () => {
    ...
    private postBattleSequenceCheck(): void {
        ...
        this.sceneData.playerMonster[this.activePlayerMonsterPartyIndex].currentHp = this.activePlayerMonster.currentHp
            ...
            onEnter: () => {
                ...
                this.sceneData.playerMonster.forEach((monster, index) => {
                    ...
                    if (index === this.activePlayerMonsterPartyIndex) {
                        ...
                        monsterMessages.push(`${this.sceneData.playerMonster[index].name} gained ${gainedExpForActiveMonster} exp.`)
                        ...
                        monsterMessages.push(`${this.sceneData.playerMonster[index].name} gained ${gainedExpForInactiveMonster} exp`)
                    }
    ...
    public handleSceneResume(sys: Phaser.Scenes.Systems, data?: BattleSceneWasResumedData | undefined) {
        ...
        this.activePlayerMonster.playMonsterDeathAnimation(() => {
            ...
            this.activePlayerMonster.switchMonster(this.sceneData.playerMonster[data.selectedMonsterIndex as number])
    ...
    private createAvailableMonstersUi(): void {
        ...
        this.sceneData.playerMonster.forEach((monster, index) => {
            ...
            const ball = this.add.image(30 * -index, 0, BATTLE_ASSET_KEYS.BALL_THUMBNAIL, 0)
                .setScale(0.8)
                .setAlpha(alpha)
