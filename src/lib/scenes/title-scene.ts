...
export class TitleScene extends BaseScene {
  ...
  create() {
    ...
    const newGameText: Phaser.GameObjects.Text = this.add
      .text(menuBgWidth / 2, 40, 'New Game', MENU_TEXT_STYLE)
      .setOrigin(0.5)
    const continueText: Phaser.GameObjects.Text = this.add
      .text(menuBgWidth / 2, 90, 'Continue', MENU_TEXT_STYLE)
      .setOrigin(0.5)
    if (!this.isContinueButtonEnabled) {
      continueText.setAlpha(0.5)
    }
    const optionsText: Phaser.GameObjects.Text = this.add
      .text(menuBgWidth / 2, 140, 'Options', MENU_TEXT_STYLE)
      .setOrigin(0.5)
    const menuContainer: Phaser.GameObjects.Container = this.add.container(0, 0, [
      menuBgContainer,
      newGameText,
      continueText,
      optionsText,
