...
export class OptionsScene extends BaseScene {
  ...
  init() {
    ...
    this.selectedBattleSceneOption = dataManager.getStore.get(
  ...
  create() {
    ...
    // Create Options
    this.add
      .text(width / 2, 40, 'Options', OPTIONS_TEXT_STYLE)
      .setOrigin(0.5)
      .setScale(1.3)
    ...
    this.optionsMenuCursor = this.add
      .rectangle(110, 75, optionMenuWidth - 20, 40, 0xffffffff, 0)
      .setOrigin(0)
  ...
  private updateTextSpeedGameObjects(): void {
    const textGameObjects: Phaser.GameObjects.Text[] =
