...
import {CONFIRMATION_MENU_OPTIONS, ConfirmationMenu} from "../../common/confirmation-menu";
...
export class MonsterPartyScene extends BaseScene {
    ...
    create() {
        ...
        this.add.tileSprite(0, 0, this.scale.width, this.scale.height, MONSTER_PARTY_ASSET_KEYS.PARTY_BACKGROUND, 0)
            .setOrigin(0)
            .setAlpha(0.7)
    ...
    private createMonster(x: number, y: number, monsterDetails: Monster): Phaser.GameObjects.Container {
        ...
        const background = this.add.image(0, 0, BATTLE_ASSET_KEYS.HEALTH_BAR_BACKGROUND)
            .setOrigin(0)
            .setScale(1.1, 1.2)
        this.monsterPartyBackgrounds.push(background)

        const leftShadowCap = this.add.image(160, 67, HEALTH_BAR_ASSET_KEYS.LEFT_CAP_SHADOW)
            .setOrigin(0)
            .setAlpha(0.5)
        const middleShadow = this.add.image(leftShadowCap.x + leftShadowCap.width, 67, HEALTH_BAR_ASSET_KEYS.MIDDLE_SHADOW)
            .setOrigin(0)
            .setAlpha(0.5)
        middleShadow.displayWidth = 285
        const rightShadowCap = this.add.image(middleShadow.x + middleShadow.displayWidth, 67, HEALTH_BAR_ASSET_KEYS.RIGHT_CAP_SHADOW)
            .setOrigin(0)
            .setAlpha(0.5)
        ...
        const healthBarTextGameObject = this.add.text(458, 95, `${monsterDetails.currentHp} / ${monsterDetails.maxHp}`, {
            fontFamily: CUSTOM_FONTS.POKEROGUE,
            color: '#ffffff',
            fontSize: '38px',
        }).setOrigin(1, 0)
        ...
        const monsterImage = this.add.image(35, 20, monsterDetails.assetKey)
            .setOrigin(0)
            .setScale(0.35)
    ...
    private goBackToPreviousScene(wasItemUsed: boolean, wasMonsterSelected: boolean): void {
        if (this.sceneData.activeMonsterKnockedOut && this.sceneData.previousSceneName === SCENE_KEYS.BATTLE_SCENE && !wasMonsterSelected) {
    ...
    private handleInputForMenu(wasBackKeyPressed: boolean, wasSpaceKeyPressed: boolean, selectedDirection: Direction): void {
    ...
    private handleInputForConfirmationMenu(wasBackKeyPressed: boolean, wasSpaceKeyPressed: boolean, selectedDirection: Direction): void {
        if (wasBackKeyPressed) {
        ...
        if (wasSpaceKeyPressed) {
            ...
            if (this.confirmationMenu.getSelectedMenuOption === CONFIRMATION_MENU_OPTIONS.YES) {
                ...
                if (this.menu.getSelectedMenuOption === MONSTER_PARTY_MENU_OPTIONS.RELEASE) {
                    this.controls.lockInput = true
                    this.infoTextGameObject.setText(`You released ${this.monsters[this.selectedPartyMonsterIndex].name} into the wild.`)
    ...
    private moveMonsters(): void {
        ...
        this.monsterPartyBackgrounds[this.monsterToBeMovedIndex] = this.monsterPartyBackgrounds[this.selectedPartyMonsterIndex]
