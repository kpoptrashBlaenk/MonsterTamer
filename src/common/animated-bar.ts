...
export class AnimatedBar {
    ...
    protected createBarShadowImages(x: number, y: number): void {
        this.leftShadowCap = this.scene.add.image(x, y, this.config.leftShadowCapAssetKey)
            .setOrigin(0, 0.5)
            .setScale(1, this.scaleY)

        this.middleShadow = this.scene.add.image(this.leftShadowCap.x + this.leftShadowCap.width, y, this.config.middleShadowCapAssetKey)
            .setOrigin(0, 0.5)
            .setScale(1, this.scaleY)
        this.middleShadow.displayWidth = this.fullWidth

        this.rightShadowCap = this.scene.add.image(this.middleShadow.x + this.middleShadow.displayWidth, y, this.config.rightShadowCapAssetKey)
            .setOrigin(0, 0.5)
            .setScale(1, this.scaleY)
